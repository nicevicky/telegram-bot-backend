...
export default async function handler(req, res) {
  ...
  try {
    logRequest(req, { endpoint: 'getWebhookInfo' });
    ...
    if (result.success) {
      res.status(200).json(createResponse(true, result.data, 'Webhook info retrieved successfully'));
    } else {
      res.status(400).json(createResponse(false, null, result.error || 'Failed to get webhook info'));
    }
